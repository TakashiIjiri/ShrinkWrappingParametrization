...
private:
  ...
  EVec3f m_cam_pos, m_cam_cnt, m_cam_up;
...
public:
  ...
  void MouseMove(const EVec2i &p)
  {
    ...
    if (m_mousebtn_state == BTN_ROT)
    {
      ...
      EVec3f axis = ((m_cam_cnt - m_cam_pos).cross(m_cam_up)).normalized();
      Eigen::AngleAxisf rotTheta = Eigen::AngleAxisf(theta, m_cam_up);
    ...
    else if (m_mousebtn_state == BTN_TRANS)
    {
      ...
      EVec3f Xdir = ((m_cam_pos - m_cam_cnt).cross(m_cam_up)).normalized();
      EVec3f t = c * dX * Xdir + c * dY * m_cam_up;
      m_cam_pos += t;
      m_cam_cnt += t;
    }
    m_mouse_pos = p;
